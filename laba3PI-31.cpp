...
public:
    ...
    bool insertNumber(int row, int col, int number) {
        ...
        if (number < 1 || number > 9) { // Исправлено: добавлены логические операторы
            throw std::invalid_argument("Неверное число. Пожалуйста, введите число от1 до 9."); // Выбрасываем исключение
        }
    ...
    bool searchNumber(int number) {
        for (int row = 0; row < SIZE; row++) {
            for (int col = 0; col < SIZE; col++) {
                if (cells[row][col] - '0' == number) { // Преобразуем символ в число для сравнения
                    std::cout << "Число " << number << " найдено в ячейке (" << row << ", " << col << ")." << std::endl;
                    return true; // Число найдено
                }
            }
        }
        std::cout << "Число " << number << " не найдено." << std::endl;
